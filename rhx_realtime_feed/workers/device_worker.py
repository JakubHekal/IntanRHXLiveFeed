...
class DeviceWorker(QtCore.QThread):
    ...
    def __init__(self, device: Device, output_sink: OutputSink,
                 project_name, project_path=None, close_device_on_stop=True):
        super().__init__()
        self.device = device
        self.output_sink = output_sink
        self.project_name = project_name
        self.project_path = project_path
        self._close_device_on_stop = close_device_on_stop
    ...
    def stop(self, timeout_ms: int = 3000) -> bool:
        ...
        if self.wait(max(0, int(timeout_ms))):
            return True
        print(f"[WORKER] DeviceWorker stop timeout after {int(timeout_ms)} ms")
        return False
    ...
    def run(self):
        ...
        try:
            ...
            if self._close_device_on_stop and self.device is not None:
                try:
                    self.device.close()
                except Exception:
                    pass
